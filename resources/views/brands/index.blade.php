...
@section('title', 'Brands List - POS System')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-xl font-bold text-gray-800">បញ្ជីម៉ាកយីហោ (Brands)</h2>
            <p class="text-xs sm:text-sm text-gray-500">គ្រប់គ្រងម៉ាកយីហោផលិតផលក្នុងស្តុក (សរុប៖ {{ $totalBrands }} ម៉ាក)</p>
        </div>
        <a href="{{ route('brands.create') }}" class="inline-flex items-center justify-center px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs sm:text-sm font-semibold rounded-xl shadow-sm shadow-blue-500/20 transition">
            <i class="fa-solid fa-plus mr-2"></i> បន្ថែមម៉ាកថ្មី
        </a>
    </div>
    ...
    <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
        <form id="filterForm" method="GET" action="{{ route('brands.index') }}" class="flex gap-3">
            ...
            <div class="relative flex-1">
                ...
                <input 
                    ...
                    placeholder="ស្វែងរកតាមឈ្មោះម៉ាកយីហោ, ការពិពណ៌នា..." 
            ...
            @if(request()->hasAny(['search', 'per_page']))
                <a href="{{ route('brands.index') }}" class="p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-xl text-xs flex items-center justify-center transition" title="កំណត់ឡើងវិញ">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <table class="w-full text-left border-collapse text-xs sm:text-sm">
                <thead>
                    <tr class="bg-gray-50/75 border-b border-gray-100 text-[11px] font-bold uppercase tracking-wider text-gray-400">
                        ...
                        <th class="py-3.5 px-6">ឈ្មោះម៉ាកយីហោ (Brand)</th>
                ...
                <tbody class="divide-y divide-gray-100 text-gray-700">
                    ...
                    <tr class="hover:bg-gray-50/60 transition">
                        ...
                        <td class="py-3.5 px-6">
                            <div class="flex items-center space-x-3">
                                <div class="w-9 h-9 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center font-bold text-xs border border-indigo-100">
                                    <i class="fa-solid fa-tag"></i>
                                </div>
                                <div>
                                    <p class="font-bold text-gray-800">{{ $brand->brand_name }}</p>
                                    <span class="text-[10px] text-gray-400">{{ $brand->created_at ? $brand->created_at->format('d/m/Y') : '-' }}</span>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="py-3.5 px-6 text-center">
                            <span class="inline-block px-2.5 py-0.5 rounded-lg text-xs font-semibold {{ $brand->is_active ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-rose-50 text-rose-700 border border-rose-100' }}">
                                {{ $brand->is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        <td class="py-3.5 px-6 text-right space-x-1">
                            <a href="{{ route('brands.edit', $brand->brand_id) }}" class="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition inline-flex items-center" title="កែប្រែ">
                                <i class="fa-regular fa-pen-to-square"></i>
                            </a>
                            <form action="{{ route('brands.destroy', $brand->brand_id) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="button" onclick="confirmDeleteBrand(this, '{{ $brand->brand_name }}')" class="p-2 text-gray-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition inline-flex items-center" title="លុប">
                    ...
                    <tr>
                        <td colspan="6" class="py-12 text-center text-gray-400">
                            <i class="fa-solid fa-tag text-3xl text-gray-300 mb-2"></i>
                            <p class="text-sm font-medium">មិនទាន់មានទិន្នន័យម៉ាកយីហោនៅឡើយ</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
        ...
        <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50 flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-gray-500">
            <div>
                បង្ហាញពី <span class="font-bold text-gray-700">{{ $brands->firstItem() ?? 0 }}</span> ដល់ <span class="font-bold text-gray-700">{{ $brands->lastItem() ?? 0 }}</span> នៃទិន្នន័យសរុប <span class="font-bold text-gray-700">{{ $brands->total() }}</span> ម៉ាក
            </div>
...
function confirmDeleteBrand(button, name) {
    Swal.fire({
        title: '<span class="text-xl font-bold text-gray-800">តើអ្នកពិតជាចង់លុបមែនទេ?</span>',
        html: `អ្នកកំពុងស្នើសុំលុបម៉ាកយីហោ <strong class="text-rose-600 font-bold">"${name}"</strong> ចេញពីប្រព័ន្ធ។`,
        icon: 'warning',
        iconColor: '#e11d48',
