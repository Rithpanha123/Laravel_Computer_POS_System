...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold text-gray-800">ចុះឈ្មោះបុគ្គលិកថ្មី</h2>
            <p class="text-xs sm:text-sm text-gray-500">បញ្ចូលព័ត៌មានលម្អិតបុគ្គលិក ជាងជួសជុល ឬអ្នកគិតលុយ</p>
        </div>
    ...
    <form action="{{ route('staff.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 sm:p-8 space-y-6">
        @csrf

        <!-- Photo Upload with Live Preview -->
        <div x-data="{ photoPreview: null }" class="flex flex-col sm:flex-row items-center gap-6 pb-6 border-b border-gray-100">
        ...
        <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" name="staff_code" value="{{ old('staff_code', 'STF-' . rand(100, 999)) }}" required class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs font-mono font-bold focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
                <div>
                    ...
                    <input type="text" name="last_name" value="{{ old('last_name') }}" placeholder="ឧ. សុខ" required class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
                <div>
                    ...
                    <input type="text" name="first_name" value="{{ old('first_name') }}" placeholder="ឧ. ពិសាល" required class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
                <div>
                    ...
                    <input type="text" name="phone" value="{{ old('phone') }}" placeholder="012 345 678" required class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-gray-700 mb-1">អ៊ីមែល (Email)</label>
                    <input type="email" name="email" value="{{ old('email') }}" placeholder="staff@example.com" class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-gray-700 mb-1">ថ្ងៃខែឆ្នាំកំណើត</label>
                    <input type="date" name="date_of_birth" value="{{ old('date_of_birth') }}" class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
            </div>
        </div>
        ...
        <div class="pt-4 border-t border-gray-100">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" min="0" name="salary" value="{{ old('salary', '0.00') }}" placeholder="0.00" class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs font-bold text-gray-800 focus:ring-2 focus:ring-blue-500 outline-none">
                </div>

                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-gray-700 mb-1">ថ្ងៃចូលបម្រើការងារ</label>
                    <input type="date" name="hire_date" value="{{ old('hire_date', date('Y-m-d')) }}" class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
            </div>
        </div>
        ...
        <div class="pt-4 border-t border-gray-100">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div class="sm:col-span-3">
                    ...
                    <input type="text" name="address" value="{{ old('address') }}" placeholder="ផ្ទះលេខ, ផ្លូវ, សង្កាត់, ខណ្ឌ..." class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-gray-700 mb-1">ឈ្មោះអ្នកទាក់ទងពេលអាសន្ន</label>
                    <input type="text" name="emergency_contact_name" value="{{ old('emergency_contact_name') }}" placeholder="ឈ្មោះសាច់ញាតិ" class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-gray-700 mb-1">លេខទូរស័ព្ទសាច់ញាតិ</label>
                    <input type="text" name="emergency_contact_phone" value="{{ old('emergency_contact_phone') }}" placeholder="012 xxx xxx" class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-gray-700 mb-1">ត្រូវជាអ្វី (Relation)</label>
                    <input type="text" name="emergency_contact_relation" value="{{ old('emergency_contact_relation') }}" placeholder="ឧ. បងប្រុស, ម្ដាយ, ឪពុក" class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
            </div>
        </div>
        ...
        <div class="pt-4 border-t border-gray-100">
            ...
            <textarea name="notes" rows="3" placeholder="ជំនាញពិសេស, បទពិសោធន៍ការងារពីមុន..." class="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs focus:ring-2 focus:ring-blue-500 outline-none">{{ old('notes') }}</textarea>
        </div>
