...
<div class="max-w-2xl mx-auto space-y-6 animate-fade-in relative">
    ...
    <div class="flex items-center justify-between stagger-item" style="animation-delay: 50ms;">
        <div>
            <h2 class="text-xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600 dark:from-blue-400 dark:to-indigo-400">ចុះឈ្មោះអតិថិជនថ្មី</h2>
            <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mt-1">បញ្ចូលព័ត៌មានអតិថិជនសម្រាប់កត់ត្រាការលក់ និងសេវាជួសជុល</p>
        </div>
    ...
    <form action="{{ route('customers.store') }}" method="POST" class="bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl rounded-3xl border border-gray-100 dark:border-slate-800 shadow-2xl shadow-blue-500/5 dark:shadow-none p-6 sm:p-8 space-y-5 stagger-item hover-3d transition-all duration-300" style="animation-delay: 150ms;">
        ...
        <div>
            ...
            <input 
                ...
                required 
                class="w-full px-4 py-2.5 bg-gray-50/75 dark:bg-slate-800/75 border border-gray-200 dark:border-slate-700 rounded-xl text-xs sm:text-sm text-gray-800 dark:text-gray-200 placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 outline-none transition"
            >
        </div>
        ...
        <div class="pt-4 border-t border-gray-100 dark:border-slate-800 flex items-center justify-end space-x-3">
            ...
            <button type="submit" class="magnetic-btn px-6 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white text-xs font-semibold rounded-xl shadow-lg shadow-blue-500/25 transition inline-flex items-center gap-1.5 transform hover:-translate-y-0.5">
...
<!-- Custom Styles for Animations, Gradients, & 3D Interactive Cards -->
<style>
@keyframes fadeIn {
